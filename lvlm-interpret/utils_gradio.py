...
def tokenizer_image_token2(prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX, return_tensors=None):
    ...
    for x in insert_separator(prompt_chunks, [image_token_index] * (offset + 1)):
        ...
        if IMAGE_TOKEN_INDEX in x:
            input_ids_extended.extend([x[1]] * 576)
        else:
            input_ids_extended.extend(x[offset:])

    if return_tensors is not None:
        if return_tensors == 'pt':
            return torch.tensor(input_ids, dtype=torch.long), torch.tensor(input_ids_extended, dtype=torch.long)
        raise ValueError(f'Unsupported tensor type: {return_tensors}')
    return input_ids


def add_text_med(state, text, image):
    ...
    state.prompt = prompt
    state.image = process_images([image], processor, model.config)[0]

    return (state, to_gradio_chatbot(state), "", None)


@spaces.GPU
def lvlm_med_bot(tokenizer, state, temperature, top_p, max_new_tokens, device_map):  
    prompt = state.prompt
    image_tensor = state.image
    image_tensor = image_tensor.unsqueeze(0).half().to(device=device_map)

    input_ids, input_ids_extended = tokenizer_image_token2(prompt, tokenizer, IMAGE_TOKEN_INDEX, return_tensors='pt')
    input_ids = input_ids.unsqueeze(0).to(device=device_map)
    input_ids_extended = input_ids_extended.unsqueeze(0).to(device=device_map)
    ...
    keywords = [stop_str]
    criteria = KeywordsStoppingCriteria(keywords, tokenizer, input_ids)
    ...
    input_text = tokenizer.decode(input_ids_list) # eg. "<s> You are a helpful ..."
    if input_text.startswith("<s> "):
        input_text = '<s>' + input_text[4:] # Remove the first space after <s> to maintain correct length
    input_text_tokenized = tokenizer.tokenize(input_text) # eg. ['<s>', '▁You', '▁are', '▁a', '▁helpful', ... ]
    input_text_tokenized[img_idx] = "average_image"

    output_ids = outputs.sequences.squeeze().tolist() 
    outputs.sequences = torch.cat((input_ids_extended, outputs.sequences), dim=1) 

    generated_text = tokenizer.decode(output_ids)
    print(generated_text)
    output_ids_decoded = [tokenizer.decode(oid).strip() for oid in output_ids] # eg. ['The', 'man', "'", 's', 'sh', 'irt', 'is', 'yellow', '.', '</s>']
    ...
    # print("Input ids", input_ids_extended.shape)
    # print("generated text tokenized", len(generated_text_tokenized))
    # print("outputs.scores:", [score.shape for score in outputs.scores])
    # print("len attentions:", len(outputs.attentions))
    # print("len attentions[0]", len(outputs.attentions[0]))
    # print("outputs.attentions:", [attention.shape for attention in outputs.attentions[0]])
    ...
    img_std = torch.tensor(processor.image_std).view(3,1,1)
    img_mean = torch.tensor(processor.image_mean).view(3,1,1)
    img_recover = image_tensor[0].cpu() * img_std + img_mean
